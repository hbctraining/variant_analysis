...
1) Create an index of the reference sequence (done once)
  1a) Create a Suffix Array Index
  1b) Generate a Burrows-Wheeler Transform from the suffix array
...
Now that we have the module load command for `bwa` in our SBATCH script, we can now add our command for running `bwa`:
...
- `/n/groups/hbctraining/variant_calling/reference/GRCh38.p7_genomic.fa` This is the path to our reference sequence.
  
- `fastq_files/synthetic_challenge_set3_normal_NGv3_1.fq.gz` and `fastq_files/synthetic_challenge_set3_normal_NGv3_2.fq.gz` These are the FASTQ sequence read files that we wish to align. In this case you can see that the reads are paired-end reads because 1) we have provided two sets of reads and 2) they have `_1`/`_2` right before the `.fq`/`.fastq` extension. This is a very common annotation for pair-end reads. Alternatively, sometimes they might also be annotated as `_R1`/`_R2`, but once again, they will almost always be placed right before the `.fq`/`.fastq` extension. We can also, note that these reads are currently compressed with gzip as annotated by the `.gz` extnesion. This is not a problem as `bwa` can read both gzip compressed and uncompressed FASTQ files.
